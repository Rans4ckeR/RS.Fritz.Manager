...
internal sealed class CaptureControlService : ICaptureControlService
{
    ...
    public async Task<FileInfo> GetStartCaptureResponseAsync(InternetGatewayDevice internetGatewayDevice, string folderPath, string filePrefix, CancellationToken cancellationToken = default)
    {
        ...
        var captureUri = new Uri(FormattableString.Invariant($"{Scheme}://{internetGatewayDevice.PreferredLocation.Host}{CapturePath}?{query}"));
    ...
    public async Task GetStopCaptureResponseAsync(InternetGatewayDevice internetGatewayDevice, CancellationToken cancellationToken = default)
    {
        ...
        var captureUri = new Uri(FormattableString.Invariant($"{Scheme}://{internetGatewayDevice.PreferredLocation.Host}{CapturePath}?iface={iface}&minor=1&type=2&capture=Stop&sid={sid}&useajax=1&xhr=1&{timeId}=nocache"));
        HttpClient httpClient = httpClientFactory.CreateClient(Constants.HttpClientName);
        HttpResponseMessage response = await httpClient.GetAsync(captureUri, HttpCompletionOption.ResponseHeadersRead, cancellationToken);
