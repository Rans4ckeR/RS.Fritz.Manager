    ...
    [property: MessageBodyMember(Name = "NewX_AVM-DE_DSLSync")] bool DslSync)
{
    public string? Status
    {
        get
        {
            ...
            if ("NONE".Equals(DslDiagnoseState, StringComparison.OrdinalIgnoreCase) && CableNokDistance is -1 && DslLastDiagnoseTime is 0 && DslSignalLossTime is > 0 and < 300 && DslActive && !DslSync)
            ...
            if ("RUNNING".Equals(DslDiagnoseState, StringComparison.OrdinalIgnoreCase) && CableNokDistance is -1 && DslLastDiagnoseTime is 0 && DslSignalLossTime > 300 && DslActive && !DslSync)
                return FormattableString.CurrentCulture($"5 minutes passed since the last {nameof(DslSync)} = 1, the Customer Premises Equipment (CPE) device is now instantiating a cable cut measurement. While the diagnose is running the {nameof(DslDiagnoseState)} is set to 'RUNNING'.");

            if ("NO_CALIB".Equals(DslDiagnoseState, StringComparison.OrdinalIgnoreCase) && CableNokDistance is -1 && DslLastDiagnoseTime is 0 && DslSignalLossTime > 300 && DslActive && !DslSync)
